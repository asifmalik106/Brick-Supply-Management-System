...
class customerModel extends Model
{
	...
	public function addOrder($brickID,$quantity,$payment,$status){
		$customerID = $_SESSION['data']['customerID'];
		date_default_timezone_set("Asia/Dhaka");
		$date = date('Y-m-d');
		$time = date('H:i:s');
		$sql = "INSERT INTO orderTable (customerID, brickID, orderQuantity, orderDate, orderTime, orderPayment, orderStatus) VALUES ('$customerID', '$brickID', '$quantity', '$date', '$time', '$payment', '$status')";
		return $this->db->execute($sql);
	}
	public function getOrderByCustomer(){
		$customerID = $_SESSION['data']['customerID'];
		$sql = "SELECT * FROM orderTable INNER JOIN brick ON orderTable.brickID = brick.brickID WHERE orderTable.customerID = '$customerID'";
		return $this->db->fetch($sql);
	}
	...
	public function updateMobile($newPhone,$customerID)
	{
		$sql = "UPDATE customer SET customerMobile = '$newPhone' WHERE customerID = '$customerID'";
		return $this->db->execute($sql);
	}
